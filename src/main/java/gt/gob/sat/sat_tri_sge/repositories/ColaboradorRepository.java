...
public interface ColaboradorRepository extends CrudRepository<SgeColaborador, String> {
    ...
    @Query(value = "select  sc.nit,\n"
            ...
            + "left join sat_tri_sge.sge_cat_dato scd on sc.tipo_tributa = scd.codigo\n"
            + "left join sat_tri_sge.sge_perfil sp on sp.id_perfil = scp.id_perfil", nativeQuery = true)
    ...
    @Query(value = "select sc.nombre, sc.nit from sat_tri_sge.sge_colaborador sc\n"
            ...
            + "where sp.id_rol = :rol and sc.tipo_tributa = :tipo", nativeQuery = true)
    ...
    @Query(value = "select sc.nit from sat_tri_sge.sge_colaborador sc\n"
            + "inner join sat_tri_sge.sge_colaborador_perfil scp on scp.nit = sc.nit\n"
            + "inner join sat_tri_sge.sge_perfil sp on sp.id_perfil = scp.id_perfil \n"
            + "where sp.id_rol = :rol", nativeQuery = true)
    String Centralizer(@Param("rol") int rol);
    ...
    @Query(value = "select sc.nombre, sc.nit from sat_tri_sge.sge_colaborador sc\n"
            + "inner join sat_tri_sge.sge_colaborador_perfil scp on scp.nit = sc.nit\n"
            + "inner join sat_tri_sge.sge_perfil sp on sp.id_perfil = scp.id_perfil \n"
            + "where sp.id_rol = :rol and sc.tipo_tributa = :tipo and sc.id_estado = 44", nativeQuery = true)
    ...
    @Query(value = "select sc.nombre, sc.nit, sgt.id_grupo, sig.id_grupo as grupo from sat_tri_sge.sge_colaborador sc\n"
            ...
            + "where sp.id_rol = :puesto and sc.tipo_tributa = :tipo and sig.id_grupo is null and sgt.id_grupo is null", nativeQuery = true)
    ...
    @Query(value = "select sc.nit, sc.nombre, sp.nombre as puesto, scd.nombre as estado from sat_tri_sge.sge_colaborador sc\n"
