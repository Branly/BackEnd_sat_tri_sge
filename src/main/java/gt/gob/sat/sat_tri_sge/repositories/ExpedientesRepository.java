...
public interface ExpedientesRepository extends CrudRepository<SgeExpediente, String> {
    ...
    @Query(value = "select se.no_expediente, scd9.nombre as TipoRecurso,\n"
            + "se.nit_contribuyente, se.fecha_ingreso, se.gerencia_origen, scd2.nombre , se.no_expediente_tributa, \n"
            + "sce.complejidad, se.cantidad_ajustes, scd4.nombre as Impuesto, sei.monto, scd3.nombre as TipoCaso,\n"
            + "scd.nombre as SubTipo, sce.fecha_interposicion, scd5.nombre as Especial, scd6.nombre as Gerencia , scd7.nombre as Departamento, sp.comentario,\n"
            + "se.fecha_preincripcion, scd8.nombre as Estado, sc2.nombre as Profecional, sc.nombre as Especialista, se.id_agenda, sa.fecha_creacion\n"
            + "from sat_tri_sge.sge_expediente se\n"
            ...
            + "where se.fecha_ingreso between to_timestamp(select ((select extract(year from (select current_date)))||'-'||(select extract(month from (select current_date))) || '-01' ))  and (select current_date)", nativeQuery = true)
    List<ReporteProjection> Report();
